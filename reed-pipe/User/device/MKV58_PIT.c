...
void PIT_Init(PITn pitn, uint32_t ms)
{
    ...
    /* 设置溢出中断时间 */
    PIT_LDVAL(pitn)  = ms * CLOCK_GetBusClkFreq() / 1000;                                       
...
void PIT_Delayus(PITn pitn, uint32_t us)
{
	...
	/* 设置溢出中断时间 */
    PIT_LDVAL(pitn)  = us * (uint32_t)(CLOCK_GetBusClkFreq() / 1000000.0);     
...
{
	return (uint32_t)((0xFFFFFFFF - PIT_CVAL(pitn)) / (CLOCK_GetBusClkFreq() / 1000000.0));
}
