...
【编译平台】IAR 8.2
【功    能】DMA 模块  用于DMA计数 和 DMA传输摄像头数据
【注意事项】
-------------------------------------------------------------------------------------------------------*/
#include "include.h"
#include "MKV58_DMA.h"
...
{ 
    ...
    DMA_CITER_ELINKNO(CHn)  = DMA_CITER_ELINKNO_CITER(IMAGEW); //当前主循环次数 
    DMA_BITER_ELINKNO(CHn) = DMA_BITER_ELINKNO_BITER(IMAGEW);   //起始主循环次数
    DMA_EN(CHn);                                              //使能通道CHn 硬件请求
...
*  函数返回：无
*  修改时间：2012-1-20
*  备    注：注意不要和摄像头的DMA通道冲突 比如摄像头的PLCK使用PORTE触发DMA 那么DMA计数就不要用PORTE管脚做触发源
*************************************************************************/
